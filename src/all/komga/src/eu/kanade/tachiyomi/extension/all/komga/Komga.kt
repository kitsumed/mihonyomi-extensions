...
open class Komga(private val suffix: String = "") : ConfigurableSource, UnmeteredSource, HttpSource() {
    ...
    private val cloudflareClientID by lazy { preferences.cloudflareClientID }
    private val cloudflareClientSecret by lazy { preferences.cloudflareClientSecret }
    private val json: Json by injectLazy()

    override fun headersBuilder(): Headers.Builder {
        val header = Headers.Builder()
            .add("User-Agent", "Mihonyomi-Komga/${AppInfo.getVersionName()}")
        if (cloudflareClientID.isNotEmpty() && cloudflareClientSecret.isNotEmpty()) {
            header.add("CF-Access-Client-Id", cloudflareClientID)
            header.add("CF-Access-Client-Secret", cloudflareClientSecret)
        }
        return header
    }
    ...
    override fun setupPreferenceScreen(screen: PreferenceScreen) {
        ...
        screen.addEditTextPreference(
            title = "Cloudflare Service Token ID",
            default = "",
            summary = cloudflareClientID.ifBlank { "Token ID" },
            key = PREF_cloudflareClientID,
        )
        screen.addEditTextPreference(
            title = "Cloudflare Service Token Secret",
            default = "",
            summary = if (password.isBlank()) "" else "*".repeat(cloudflareClientSecret.length),
            inputType = InputType.TYPE_CLASS_TEXT or InputType.TYPE_TEXT_VARIATION_PASSWORD,
            key = PREF_cloudflareClientSecret,
        )
    }
    ...
    private val SharedPreferences.cloudflareClientID
        get() = getString(PREF_cloudflareClientID, "")!!

    private val SharedPreferences.cloudflareClientSecret
        get() = getString(PREF_cloudflareClientSecret, "")!!
    ...
    companion object {
        ...
        private const val PREF_cloudflareClientID = "Cloudflare Service Token ID"
        private const val PREF_cloudflareClientSecret = "Cloudflare Service Token Secret"
